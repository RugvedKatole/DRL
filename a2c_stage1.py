...
from model.a2c import a2c_update_stage1, generate_train_data
from model.a2c import generate_action
from model.a2c import transform_buffer
        ...
        while not terminal and not rospy.is_shutdown():
            ...
            if env.index == 0:
                ...
                if len(buff) > HORIZON - 1:
                    ...
                    a2c_update_stage1(policy=policy, optimizer=optimizer, batch_size=BATCH_SIZE, memory=memory,
                                            epoch=EPOCH, coeff_entropy=COEFF_ENTROPY, clip_value=CLIP_VALUE, num_step=HORIZON,
                                            num_env=NUM_ENV, frames=LASER_HIST,
                                            obs_size=OBS_SIZE, act_size=ACT_SIZE,value_loss_coef=value_loss_coef,max_grad_norm=max_grad_norm)
...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        if acktr:
            opt = KFACOptimizer(policy)
        else:
            opt = optim.RMSprop(
                policy.parameters(), lr, eps=eps, alpha=alpha)
        mse = nn.MSELoss()
